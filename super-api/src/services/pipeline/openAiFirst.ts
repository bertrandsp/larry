...
export async function runOpenAiFirstPipeline(
  ...
  try {
    ...
    const validatedOpenAiResponse = validateOpenAiResponse(
    ...
    console.log(
      `✅ Generated ${validatedOpenAiResponse.terms.length} terms with OpenAI`,
    );

    // Step 2: Convert OpenAI terms to ValidatedTerm format
    const openAiTerms: ValidatedTerm[] = validatedOpenAiResponse.terms.map(
      (term) => ({
        term: term.term,
    ...
    const enrichedTerms = await enrichWithExternalSources(
      openAiTerms,
      params.topic,
    );
    ...
    const finalResult = postProcessValidation(
      validationResult,
      0.3,
      params.numTerms,
    );
    ...
    const termsEnriched = enrichedTerms.filter(
      (term) => term.source !== "AI Generated" && term.confidenceScore > 0.6,
    ).length;

    const enrichmentSuccessRate =
      openAiTerms.length > 0 ? (termsEnriched / openAiTerms.length) * 100 : 0;
    ...
    console.log(
      `✅ OpenAI-first pipeline completed: ${result.terms.length} terms, ${result.facts.length} facts`,
    );
    console.log(
      `📊 Enrichment stats: ${termsEnriched}/${openAiTerms.length} terms enriched (${enrichmentSuccessRate.toFixed(1)}%)`,
    );
      ...
      try {
        ...
        if (externalTerm) {
          ...
          const isHighQualitySource =
            externalTerm.source === "Merriam-Webster" ||
            (externalTerm.source === "Wiktionary" &&
              externalTerm.confidence > 0.8);
          ...
          if (isHighQualitySource) {
            ...
            return {
              ...
              examples:
                externalTerm.examples.length > 0
                  ? externalTerm.examples
                  : term.examples,
  ...
  for (const term of terms) {
    ...
    if (!seen.has(normalized)) {
      ...
      const existingIndex = merged.findIndex(
        (t) => t.term.toLowerCase().trim() === normalized,
      );
      if (existingIndex !== -1) {
        const existing = merged[existingIndex];
        if (
          term.confidenceScore > existing.confidenceScore ||
